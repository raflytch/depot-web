        ...
        <main className="pt-20 pb-8 px-4 lg:py-8 lg:pl-5">
            ...
            <section className="overflow-x-scroll lg:overflow-x-clip">
                <div className="flex gap-12 p-6 min-w-fit">
                    <span
                        className={"py-2 px-1 cursor-pointer font-semibold transition duration-200 " + (viewState === "ALL" ? activeClassName : "")}
                        onClick={() => setViewState("ALL")}>All</span>
                    <span
                        className={"py-2 px-1 cursor-pointer font-semibold transition duration-200 " + (viewState === "SUCCESS" ? activeClassName : "")}
                        onClick={() => setViewState("SUCCESS")}>Success</span>
                    <span
                        className={"py-2 px-1 cursor-pointer font-semibold transition duration-200 " + (viewState === "PENDING" ? activeClassName : "")}
                        onClick={() => setViewState("PENDING")}>Pending</span>
                    <span
                        className={"py-2 px-1 cursor-pointer font-semibold transition duration-200 " + (viewState === "FAILED" ? activeClassName : "")}
                        onClick={() => setViewState("FAILED")}>Failed</span>
                </div>
            </section>
            <section className="">
                    ...
                    <Table.Body className="divide-y">
                        {showedProducts.map((payment, index) => (
                            <Table.Row key={index + 1}>
                                ...
                                <Table.Cell>{new Date(payment.timestamp).toLocaleString()}</Table.Cell>
                                <Table.Cell>{payment.product.name}</Table.Cell>
                                <Table.Cell>{payment.amount}</Table.Cell>
                                <Table.Cell className={"font-semibold " + ((payment.status === "PENDING") ? "text-yellow-400" : (payment.status === "SUCCESS" ? "text-green-400" : "text-red-700"))}>{payment.status}</Table.Cell>
                            </Table.Row>
                        ))}
                    </Table.Body>
                </Table>
